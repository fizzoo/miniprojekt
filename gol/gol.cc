...
static const char* RPENTOMINO =
" OO"
"OO "
" O ";

static const char* SMALLBOOM =
"OOO"
"O O"
"O O";
...
struct double_xy{
    ...
    int int_x(){
        return (int(SIZEX + SIZEX*x + 0.01))/2; // 0.01 because rounding often made em SLIGHTLY below the real position
    }
    int int_y(){
        return (int(SIZEY - SIZEY*y + 0.01))/2;
    }
};
...
struct WindowScale {
    ...
    double l {-1.0};
    double r { 1.0};
    const double scalefactors[SCALESIZE] = {1.0, 1.4, 2.0, 2.8, 4.0, 5.6, 8.0, 11.3, 16.0, 22.6, 32.0};
...
struct Board {
    std::vector<unsigned char> aliveactive = std::vector<unsigned char>(SIZEX * SIZEY, 0);
    std::vector<unsigned char> alivewait = std::vector<unsigned char>(SIZEX * SIZEY, 0);
    std::vector<unsigned char> savedstate = std::vector<unsigned char>(SIZEX * SIZEY, 0);
    ...
    void save(){
        savedstate = aliveactive;
    }

    void load(){
        aliveactive = savedstate;
    }
    ...
    bool aliveat(int x, int y) {
        return aliveactive[y * SIZEX + x] == 255;
    }
    ...
    void run(){
        ...
        for (int i = 0; i < NRTHREADS; ++i){
            pool([=](){
                    for (int y = i*each; y < (i+1)*each || ((i+1) == NRTHREADS && y < SIZEY); y++){
...
"in vec2 texcoord;\n"
"out vec2 Texcoord;\n"
"void main(){Texcoord = texcoord; gl_Position = vec4(position, 0.0, 1.0);}\n";
...
"void main(){vec4 inp = texture(tex, Texcoord); if (inp.r > 0.95 || inp.r < 0.01) {outColor = vec4(inp.r, inp.r, inp.r, inp.r);} else {outColor = vec4(sin(inp.r*3.14), 0.0, sin(inp.r*32)*0.4, inp.r);}}\n";
...
struct GLstate {
    ...
    WindowScale loc;
    float vertices[16] {
        //  Position      Texcoords (bestäms av setvert)
    ...
    void draw(){
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, SIZEX, SIZEY, 0, GL_RED, GL_UNSIGNED_BYTE, global_b.aliveactive.data());
    ...
    void sdlglewinit(){
        ...
        window = SDL_CreateWindow("fizzos Game of Life", 100, 100, SIZEX, SIZEY, SDL_WINDOW_OPENGL);
    ...
    void vabetexinit(){
        //VAO, (shaders, vbo, etc blir bunden till denna, KRÄVS tydligen för flesta graffe)
        GLuint vao;
    ...
    void shadersinit(){
        ...
        //assert(errvar != GL_INVALID_ENUM); //Kan få ändå?! jobbig
        ...
        //data layout, allt "in" i shaderna.
...
int main(int argc, const char* argv[]){
    {
        if (argc == 3) {
            ...
            SIZEY = atoi(argv[2]);
            if (!SIZEX || !SIZEY) {
                std::cout << "Incorrect input parameters, atoi didn't work on: '" << argv[1] << "', '" << argv[2] << "'" << std::endl;
                return 1;
            }
        }
    }
